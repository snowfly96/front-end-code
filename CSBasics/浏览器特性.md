...
单进程浏览器指的是留啦你的所有功能模块都是运行在同一个进程中,即html解析、网络请求、js代码执行、内容渲染都在同一个进程中执行,导致浏览器不稳定、不流畅和不安全
...
**渲染流程:** 构建DOM树、计算样式、布局(构建布局树)、分层(构建分层树)、绘制(构建绘制列表)、分块(将视口内每个图层进行分块)、光栅化(将图块转化成位图)、合成(合成现成进行合成并显示)
